...
PY
    ...
    echo ""
    echo "Step 7: FOD and 100k tractography using dwi2mask, QC-only version"
    ...
    if [ -f "${T2}" ] && [ -n "${T2MASK}" ] && [ -f "${T2MASK}" ]; then
      ...
      echo ""
      echo "Step 11: FOD and 100k tractography using manual T2 mask in b0"
