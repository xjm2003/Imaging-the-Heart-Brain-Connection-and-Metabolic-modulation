...
ids = [x.strip() for x in open("ids_connectome_global_17.txt") if x.strip()]

meta = pd.read_csv("metadata_connectome_17_clean.csv", dtype=str).fillna("")
feat = pd.read_csv("connectome_subject_summary_17.csv", dtype=str).fillna("")
card = pd.read_csv("metadata_sources/CardiacMetrics_MWMResults_GLP1Mice_merged.csv", dtype=str).fillna("")
...
# keep only actual 17 connectome IDs
meta = meta[meta["ID"].isin(ids)].drop_duplicates("ID", keep="first").copy()
feat = feat[feat["ID"].isin(ids)].drop_duplicates("ID", keep="first").copy()

# normalized merge keys
meta["BadeaID_norm"] = meta["BadeaID"].map(norm_id)
card["Cardiac_ID_norm"] = card["ID"].map(norm_id)

# avoid duplicate cardiac rows
card = card.drop_duplicates("Cardiac_ID_norm", keep="first").copy()

# prefix selected cardiac/MWM columns
cardiac_vars = [
    "Mass",
    "Age",
    "Diastolic_LV_Volume",
